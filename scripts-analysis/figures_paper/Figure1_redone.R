...
data <- read.csv(here::here("chain-output","merged-csvs",paste0(model,"_",pref,"_concatenated_files_2.csv")))
data  <- data[data$Mutation == mut & data$Selection == sel,]
data=data[data$Step < 12 ,]
liststat <- data[data$Step < 12 ,]  #statement before 15 steps
keeps=tapply(liststat$Count,liststat$ID,max)>3 #statements with more than 1 selection
liststat  <- unique(liststat[,c("ID","Statement")])[keeps,]
liststat <- liststat[-c(2:10),] #remove the initial bet and the one who disapear
...
plot(1,1,ylim=range(data[["Count"]])*1.02,xlim=c(0,11),type="n",ylab="Number of Time Selected",xlab="Step")
for(u in ids){
