...
for(post in params[2]){
  ...
  par(xpd=T)
    ...
    fields::image.plot(legend.only=T,zlim=c(0,2),col=pal(100),horizontal=T,smallplot=c(.80,.90,.84,.86),axis.args=list(cex.axis=.8,a=c(0,1,2)),legend.args=list(text=expression("content-dependent bias"~(beta)),side=3,line=.4))
}
dev.off()
