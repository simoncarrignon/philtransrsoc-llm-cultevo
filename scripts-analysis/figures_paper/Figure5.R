...
names(metrics) <- c("anxiety","negative emotions","positive emotions")
png("Figure5.png",width=2400,height=1300,pointsize=17)
par(mfrow=c(3,3),mar=c(4,4,2,1),cex=1.2,oma=c(0,0,0,7))
for(mi in names(models)[c(1,2,4)]){
for(nme in names(metrics)){
        me  <-  metrics[nme]
		m <- models[mi]
        if(me == "posemo")ylim <- c(0,0.25)
        else ylim <- c(0,0.025)
		dt <- read.csv(here::here("text_analysis/output/summary_tables/",gsub("generate_new_statements","gennew_concatenated",paste0(m,"_",me,".csv"))))
		dt$Selection  <-  as.factor(dt$Selection)
		dt$Mutation  <-  as.factor(dt$Mutation)
		plot(1,type="n",xaxt="n",pch=20,col=cols["original"],ylab=nme,xlim=c(.7,4.3),main='',xlab="Selection",ylim=ylim)
