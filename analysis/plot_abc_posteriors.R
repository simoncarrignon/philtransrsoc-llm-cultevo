...
for(mv in models){
    for( ge in names(file_results)){
       strat <- paste(mv,ge)
       if(length(list_allposteriors[[gsub(" ","_",tolower(strat))]])==0)next;
       ...
       uu=legend("topright",legend=mut_names,pt.bg=cols,pch=21,title="Mutation operator",bty="n",cex=.8)
       legend(x=(uu$rect$left),y=uu$rect$top-(uu$rect$h*1.4),legend=sel_names,pt.bg=0,pch=pchs,title="Selection operator",bty="n",cex=.8)
