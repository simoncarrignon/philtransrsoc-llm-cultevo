...
for(mi in names(models)[-c(5,3)][c(1,3,2)]){
    ...
    plot(1,type="n",xaxt="n",pch=20,ylim=c(0,2.5),ylab=expression("frequency-dependent bias (J)"),xlim=c(.7,4.3),main='',xlab="Selection")
...
for(mi in names(models)[-c(5,3)][c(1,3,2)]){
    ...
    m_allmodes$Selection  <-  as.factor(m_allmodes$Selection)
    m_allmodes$Mutation  <-  as.factor(m_allmodes$Mutation)
    plot(1,type="n",xaxt="n",pch=20,col=cols["original"],ylim=c(0,2.5),ylab=expression("content-dependent bias"~(beta)),xlim=c(.7,4.3),main='',xlab="Selection")
    mtext(mi,side=3,line=.5,adj=0,font=2,cex=1.1)
    grid(lty=1,nx=0)
    abline(v=1:4,col=adjustcolor("grey",.6))
    abline(h=0:7,col=adjustcolor("grey",.6))
